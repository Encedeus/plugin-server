...
func (sq *SourceQuery) QueryPlugin() *PluginQuery {
	query := (&PluginClient{config: sq.config}).Query()
...
func (sq *SourceQuery) Clone() *SourceQuery {
	...
	return &SourceQuery{
		...
		withPlugin: sq.withPlugin.Clone(),
...
func (sq *SourceQuery) WithPlugin(opts ...func(*PluginQuery)) *SourceQuery {
	query := (&PluginClient{config: sq.config}).Query()
...
//
//	client.Source.Query().
//		GroupBy(source.FieldRepository).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (sq *SourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Source, error) {
	...
	if query := sq.withPlugin; query != nil {
		if err := sq.loadPlugin(ctx, query, nodes,
			func(n *Source) { n.Edges.Plugin = []*Plugin{} },
			func(n *Source, e *Plugin) { n.Edges.Plugin = append(n.Edges.Plugin, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SourceQuery) loadPlugin(ctx context.Context, query *PluginQuery, nodes []*Source, init func(*Source), assign func(*Source, *Plugin)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(plugin.FieldSourceID)
	}
	query.Where(predicate.Plugin(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(source.PluginColumn), fks...))
	}))
